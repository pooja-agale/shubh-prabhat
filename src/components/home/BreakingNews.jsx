...
const BreakingNews = () => {
  ...
  const handlePrev = () => {
    ...
    setTimeout(() => {
      setCurrentIndex(
        (prev) => (prev - 1 + breakingData.length) % breakingData.length
      );
      setFade(true);
    }, 300);
  };
  ...
  return (
    <div className="relative h-screen font-marathi overflow-hidden">
      ...
      {/* Slide Content */}
      <div
        className={`absolute top-0 left-0 w-full h-full flex flex-col lg:flex-row items-center justify-center lg:flex lg:justify-evenly px-6 text-center lg:text-left transition-opacity duration-700 ${
          fade ? "opacity-100" : "opacity-0"
        }`}
      >
        <div className="mb-6 lg:mb-16">
          <h1 className="text-white lg:text-7xl ... whitespace-pre-wrap leading-tight text-3xl">
            {currentSlide.headline}
          </h1>
          <p className="text-white lg:text-3xl ... whitespace-pre-line text-md">
            {currentSlide.desc}
          </p>
        </div>
